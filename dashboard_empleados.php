...
<?php include 'Consultas-php/obtener_empleados.php'; ?>
<!DOCTYPE html>
<html lang="es">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Sistema de Barberia - Panel Lista de empleados</title>
    ...
    <body>
        ...
        <div class="header">
            <div class="breadcrumbs">
                <a href="#">Sistema de barberia</a> / <a href="#">lista de empleados</a>
            </div>
            <div class="user-info">
                ...
                <tbody>
                    <?php if ($resultado && $resultado->num_rows > 0): ?>
                        <?php while($fila = $resultado->fetch_assoc()): ?>
                            <tr>
                                <td><?= htmlspecialchars($fila['id_empleados']) ?></td>
                                <td><?= htmlspecialchars($fila['Nombre_completo']) ?></td>
                                <td><?= htmlspecialchars($fila['telefono']) ?></td>
                                <td><?= htmlspecialchars($fila['fecha_ingreso']) ?></td>
                        
                                <td>
                                    <a class="btn editar" href="editar_cita.php?id=<?= $fila['id_empleados'] ?>">Elimar</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
